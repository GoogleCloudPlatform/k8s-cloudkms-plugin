...
func (h *HealthZ) pingKMS(ctx context.Context, c KeyManagementServiceClient) error {
	...
	encryptRequest := EncryptRequest{Uid: uuid.NewString(), Plaintext: []byte(plainText)}
	...
	decryptRequest := DecryptRequest{Uid: uuid.NewString(), Ciphertext: []byte(encryptResponse.Ciphertext)}
